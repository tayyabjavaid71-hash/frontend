...
import {
  Plus, Pencil, Trash2, Eye, EyeOff, ArrowUp, ArrowDown, Loader2, ImageIcon,
} from 'lucide-react';
import {
  getAllBannersAdmin,
  deleteBanner,
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Manage homepage hero banners</p>
        </div>
        <Link
        ...
        <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-100">
              <tr>
                <th className="text-left px-5 py-3.5 font-semibold text-slate-600">Order</th>
                <th className="text-left px-5 py-3.5 font-semibold text-slate-600">Preview</th>
                ...
                <th className="text-left px-5 py-3.5 font-semibold text-slate-600">Buttons</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {banners.map((banner, idx) => (
                <tr key={banner.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  {/* Title */}
                  <td className="px-5 py-4">
                    <p className="font-semibold text-slate-900 truncate max-w-[200px]">{banner.title}</p>
                    {banner.subtitle && (
                      <p className="text-slate-400 text-xs mt-0.5 truncate max-w-[200px]">{banner.subtitle}</p>
                    )}
                  </td>
                  ...
                  <td className="px-5 py-4">
                    <span className="text-slate-500">
                      {banner.banner_buttons?.length ?? 0} btn{(banner.banner_buttons?.length ?? 0) !== 1 ? 's' : ''}
                    </span>
                  </td>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button
                        ...
                        className="p-2 rounded-lg bg-slate-100 hover:bg-red-500 hover:text-white transition-colors disabled:opacity-50"
                        title="Delete"
                      >
                        {deleting === banner.id
